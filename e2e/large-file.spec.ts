...
const LARGE_FILE_PATH = path.join(__dirname, "mock", "large-file.mov");
...
test.describe("Large File Real-World Testing", () => {
  ...
  test.skip(!fs.existsSync(LARGE_FILE_PATH), "skipped: large-file.mov not found");

  test("handles real large file (large-file.mov) without hanging", async ({ page }) => {
    ...
    console.log(`Testing with large-file.mov: ${fileSizeInMB}MB`);
    ...
    await expect(page.getByText("large-file.mov")).toBeVisible();
